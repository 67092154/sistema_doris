...
const VentasFerreteria = () => {
    ...
    const fetchProducts = () => {
        axios.get(`${endpoint}/productos`)
            .then(response => {
                const ferreteria = response.data.filter(product => product.tipo === 'Material de Ferretería');
                setProducts(ferreteria);
            })
            .catch(error => {
    ...
    const fetchSales = () => {
        axios.get(`${endpoint}/ventas_ferreteria`)
            .then(response => {
    ...
    const handleCISearch = (e) => {
        ...
        setCustomerCI(ci);
        if (ci.length > 0) {
            axios.get(`${endpoint}/ventas_ferreteria/clientes/${ci}`)
                .then(response => {
                    if (response.data) {
    ...
    const confirmSale = (showPdf) => {
        ...
        axios.post(`${endpoint}/ventas_ferreteria/confirm`, { ventas })
            .then(response => {
    ...
    return (
        <div className="ventas-container">
            <Toast ref={toast} />
            <h1>Venta de Material de Ferretería</h1>
            ...
            <Dialog header="Resumen de Venta" visible={visible} footer={renderFooter} onHide={() => setVisible(false)}>
                <div className="customer-info">
                    ...
                    <InputText ref={ciInputRef} placeholder="C.I." value={customerCI} onChange={handleCISearch} />
            ...
            <Dialog header="Confirmación de Venta" visible={confirmVisible} onHide={() => setConfirmVisible(false)} footer={
                <div>
                    <Button label="Sin Recibo" icon="pi pi-times" onClick={() => confirmSale(false)} className="p-button-secondary" />
                    <Button label="Sí" icon="pi pi-check" onClick={() => confirmSale(true)} />
                </div>
            }>
...
export default VentasFerreteria;
